...
def main():
    ...
    db.session.query(db.Block).filter(db.Block.prev_block_hash == 32 * '\x00').update(values={'depth': 0})
    ...
    while found:
        ...
        for block_id, block_hash, depth in (
            ...
            while True:
                depth += 1
                res = db.session.query(db.Block.id, db.Block.block_hash).filter(db.Block.prev_block_id == block_id).first()
                if res is None:
                    break
                #prev_block_id = block_id
                block_id, block_hash = res
                db.session.query(db.Block).filter(db.Block.id == block_id).update(values={'depth': depth})  # , 'prev_block_id': prev_block_id})
                #if depth % 20 == 0:
                #    db.session.commit()
                db.session.commit()
